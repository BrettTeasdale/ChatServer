defmodule ChatServerWeb.ChatLive.FindServerModalComponent do
  ...
  def render(assigns) do
    ...
    <div id="inner_find_server_form">
      ...
      >
        <div class="w-full relative">
          <.input
            field={@form[:query]}
            type="text"
            class="w-full px-4 py-3 border-2 border-gray-200 rounded-lg focus:outline-none focus:border-green-500 peer"
            required
          />
          ...
          <svg
            ...
            class="absolute right-4 top-2.5 text-gray-400"
          >
      ...
      <div
        class="flex flex-col overflow-y-auto max-h-[calc(100vh-250px)]"
        phx-update="stream"
        id="server_search_results"
        phx-hook="findServerScroll"
        phx-target={@myself}
      >
        <div
          :for={{dom_id, server} <- @streams[:search_results]}
          id={dom_id}
          data-server_id={server.id}
          class="search_result w-full"
        >
          <div class="font-semibold">
            {server.name}
            <%= if Enum.any?(@server_users, fn su -> su.server_id == server.id end) do %>
              <button
                class="float-right bg-red-500 text-white px-3 py-1 rounded-lg hover:bg-red-600 mt-2"
            ...
            <% else %>
              <button
                class="float-right bg-green-500 text-white px-3 py-1 rounded-lg hover:bg-green-600 mt-2"
  ...
  def handle_event("search", %{"server" => server_params}, socket) do
    ...
    socket =
      socket
      |> assign(:query, query)
      |> stream(:search_results, Servers.search_servers(query, socket.assigns.search_page_size),
        reset: true
      )
  ...
  def previous_page(socket, {:previous_page, last_server_id}) do
    ...
    previous_page_servers =
      Servers.search_servers_previous(
    ...
    if previous_page_servers != [] do
      Enum.reduce(previous_page_servers, socket, fn server, acc_socket ->
        stream_insert(acc_socket, :search_results, server,
          at: 0,
          limit: 2 * socket.assigns.page_size
        )
      end)
      |> assign(:last_viewport_event, System.monotonic_time())
    else
  ...
  def next_page(socket, {:next_page, last_server_id}) do
    ...
    next_page_servers =
      Servers.search_servers_next(socket.assigns.query, last_server_id, socket.assigns.page_size)
